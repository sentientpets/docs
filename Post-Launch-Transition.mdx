...
<Frame>
  <img src="/images/transition-timeline.png" alt="GOCHI Transition Timeline" />
</Frame>

<Steps>
  <Step title="Pre-Launch Final Week">
    - Final points accumulation period
    - Activity requirement verification
    - Special pre-launch events and bonuses
    - Community preparation for launch
  </Step>
  <Step title="Points Snapshot (T-24h)">
    - Final snapshot of all player points
    - Points accumulation ends
    - Reward system temporarily paused
    - Calculation of token allocations
  </Step>
  <Step title="Token Launch Day">
    - $PURR token officially launches
    - Initial 50% token distribution to qualified accounts
    - Trading begins on supported exchanges
    - Transaction tax system activates
  </Step>
  <Step title="Reward Transition (T+48h)">
    - SOL reward system activates
    - First reward distribution from transaction taxes
    - Full transition to care-to-earn mechanics
    - Vesting schedule begins for remaining tokens
  </Step>
</Steps>
...
At launch, your accumulated points convert to $PURR tokens according to the allocation formula:

```
IndividualTokenAllocation = (YourPoints ÷ TotalPointsPool) × 150,000,000 $PURR
```
...
<CardGroup cols={3}>
  <Card title="Funding Source" icon="money-bill-transfer">
    5% of all $PURR token transaction taxes fund the reward pool
  </Card>
...
After launch, creating new pets requires $PURR tokens:

```mermaid
graph TD
    A[New User] -->|Create Pet| B{Has $PURR Tokens?}
    B -->|Yes| C[Burn Tokens]
    C --> D[Create New Pet]
...
<CardGroup cols={2}>
  <Card title="Creation Requirement" icon="paw">
    Creating a new pet requires burning a specified amount of $PURR tokens:
    
    - Initial cost: 5,000 $PURR
...
<Steps>
  ...
  <Step title="Prepare Wallet">
    Ensure your connected wallet is properly configured to receive $PURR tokens and SOL rewards.
  </Step>
